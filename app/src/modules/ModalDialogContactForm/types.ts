 ...
 * @property {(ContactFormModal | ContactFormModal[])} [data] - the data used to create the contact form modal.
...
export type ModalDialogContactFormProps = {
  ...
  modalId: string;
  data?: ContactFormModal | ContactFormModal[];
  csrfToken?: string;
};
 ...
 * @property {({key: string; text: string}[] | undefined)} [errorMessage] - Error messages associated with input validation.
 ...
 * @al-dev93
 */
export type FormContentProps = Omit<FormProps, 'idForm' | 'setShowAlert'>;
 ...
 * @property {boolean} [inEdition] - Indicates whether the input field is in edition mode.
 * @property {boolean} [isHovered] - Indicates whether the input field is currently hovered.
 * @property {({key: string; text: string}[] | undefined)} [errorMessage] - The error message showing in the popover.
 *
 * @al-dev93
 */
